                        ...
                        if (nodeYaml) {
                            ...
                            nodeYaml.value!.split("\n").forEach((line) => {
                                ...
                                if (yamlKey === 'title') {
                                    ...
                                    headerContent += `<h1 id="${id}" style="font-weight:bold;text-transform:uppercase"><a style="color:inherit;text-decoration:none" onclick={(event)=>{event.preventDefault();document.querySelector(".header-svx")?.scrollIntoView()}} href="/">${value}</a></h1>`
                                } else if (yamlKey === 'subtitle') {
                                    const value = yamlValue.trim();
                                    const id = makeId(value);
                                    headerContent += `<h2 id="${id}" style="font-weight:bold"><a style="color:inherit;text-decoration:none" onclick={(event)=>{event.preventDefault();document.querySelector(".container-toc")?.scrollIntoView()}} href="/">${value}</a></h2>`
                                } else if (yamlKey === 'author') {
                                    const value = yamlValue.trim();
                                    headerContent += `<h3 class="author" style="font-weight:normal">By ${value}</h3>`;
                                } else if (yamlKey === 'timeline') {
                                    headerContent += `<I p="./timeline/Menu.svelte" />`;
                                }
                            });
